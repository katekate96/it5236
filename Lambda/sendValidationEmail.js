				...
				if (err) {
					...
					if(err.errno == 1062) {
						...
						if(err.sqlMessage.indexOf('emailvalidationid') != -1) {
							// This should be a "Internal Server Error" error
							callback(formatErrorResponse('BAD_REQUEST', ["Emailvalidationid already exists"]));
						} 
					} else {
						// This should be a "Internal Server Error" error
						callback(formatErrorResponse('INTERNAL_SERVER_ERROR', [err]));
				        	...
				        	console.log("successful new emailvalidationid");
			      			callback(null,"new emailvalidationid successful");
			      				setTimeout(function(){
			      				conn.end();
			      			}, 3000);
	      		}
